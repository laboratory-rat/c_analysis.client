...
```js
// CommonJS
const {MDCSlider} = require('../mdc-slider/dist/mdc.slider');

// AMD
require(['/path/to/../mdc-slider/dist/mdc.slider'], ({MDCSlider}) => {
  // Use MDCSlider
});
...
The `../mdc-slider` package ships with an `MDCSliderFoundation` class that framework authors can
